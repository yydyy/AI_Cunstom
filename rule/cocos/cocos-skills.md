...
### 3.1 版本意识
- 必须区分 CC 2.4.x（`cc.Node`/`cc.Vec2`）与 3.x（`node`/`Vec3`/`setPosition`）语法
- 优先 TypeScript 严格类型，避免 `any`

### 3.2 组件使用规范
- 使用 Cocos 组件前先查阅对应版本 API 文档并确认参数与生命周期语义
- 禁止凭经验臆测调用
...
### 3.4 同步架构
- 坚持逻辑层与表现层分离
- 帧同步/状态同步保证确定性逻辑（警惕浮点误差）

### 3.5 安全护栏
- 异步回调（如 `resources.load`、`scheduleOnce` 后）执行前，优先检查 `this.node.isValid`

### 3.6 禁止扩展系统类型
- **严格禁止**：不要在引擎系统类型（如 `cc.Node`、`cc.Component` 等）上新增自定义字段
- ❌ 错误：`node.age = 10`
- ✅ 正确：仅在自定义类型组件上新增字段
